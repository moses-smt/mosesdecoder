...
TranslationTable::TranslationTable(const string &fileName, IndexType *targetIndex)
...
Classifier::AlignmentType TranslationTable::GetAlignment(const std::string &alignStr)
{
  AlignmentType out;
...
size_t TranslationTable::AddPhrase(const string &phrase, IndexType *index)
{
  ...
  if (it != index->left.end()) {
    ...
    id = index->size() + 1; // index is one-based because of VW (XXX legacy)
