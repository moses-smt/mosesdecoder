...
Options:
  ...
  --Hierarchical              ... Tune with hierarchical options                 
";
  exit 1;
}
...
#   $cmd = "$mert_extract_cmd $mert_extract_args --scfile $score_file.BLEU --ffile $feature_file.BLEU --sctype BLEU -r ".join(",", @references)." -n $nbest_file";
#   print STDERR "executing $cmd\n";
# 
#   safesystem("date"); 
# 
#   if (defined $___JOBS) {
#     safesystem("$qsubwrapper $pass_old_sge -command='$cmd' -queue-parameter=\"$queue_flags\" -stdout=extract.out.BLEU -stderr=extract.err.BLEU" )
#       or die "BLEU Failed to submit extraction to queue (via $qsubwrapper)";
#   } else {
#     safesystem("$cmd > extract.out.BLEU 2> extract.err.BLEU") or die "BLEU Failed to do extraction of statistics.";
#   }
# 
#   $cmd = "$mert_extract_cmd $mert_extract_args --scfile $score_file.TER --ffile $feature_file.TER --sctype TER -r ".join(",", @references)." -n $nbest_file";
#   print STDERR "executing : $cmd\n";
# 
#   safesystem("date");
# 
#   if (defined $___JOBS) {
#     safesystem("$qsubwrapper $pass_old_sge -command='$cmd' -queue-parameter=\"$queue_flags\" -stdout=extract.out.TER -stderr=extract.err.TER" )
#       or die "TER Failed to submit extraction to queue (via $qsubwrapper)";
#   } else {
#     safesystem("$cmd > extract.out.TER 2> extract.err.TER") or die "TER Failed to do extraction of statistics.";
#   }
#   open(FILE,">merge.init") || die ("File creation ERROR : merge.init");
#   print FILE "BLEU 1 $score_file.BLEU $feature_file.BLEU\n";
#   print FILE "TER 4 $score_file.TER $feature_file.TER\n";
#   close(FILE);
#   
  print STDERR "\n";
  safesystem("date");
  ...
  print STDERR "executing : \\cp -f $weights_in_file run$run.$weights_in_file\n";
  ...
  $cmd = $cmd." --sctype MERGE --ifile run$run.$weights_in_file";
 ...
 # backup copies
#   safesystem ("\\cp -f extract.err.TER run$run.extract.err.TER") or die;
#   safesystem ("\\cp -f extract.out.TER run$run.extract.out.TER") or die;
#   safesystem ("\\cp -f extract.err.BLEU run$run.extract.err.BLEU") or die;
#   safesystem ("\\cp -f extract.out.BLEU run$run.extract.out.BLEU") or die;
...
#     print STDERR $scorer_name."\n";
#     print STDERR $scorer_weight."\n";
    safesystem ("\\cp -f extract.out.$scorer_name run$run.extract.out.$scorer_name") or die;
#     print FILE "$scorer_name $scorer_weight $score_file.$scorer_name $feature_file.$scorer_name\n";
  }
  safesystem ("\\cp -f merge.init run$run.merge.init") or die;
  safesystem ("\\cp -f mergeWeight.err.MERGE run$run.mergeWeight.err.MERGE") or die;
  safesystem ("\\cp -f mergeWeight.out.MERGE run$run.mergeWeight.out.MERGE") or die;
