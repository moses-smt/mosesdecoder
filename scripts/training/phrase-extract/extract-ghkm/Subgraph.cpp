...
/*
        else//child is from frontier set
		{if (!allgrandChildrenSource(child))
            m_expandedNodes.insert(child);
		else{
			addGrandChildrenToExpandedNodes(child); // frontier graph that produces lexical rule
...
/*
 bool Subgraph::allgrandChildrenSource(Node* child){ 

	std::vector<Node*> grandChildren=child->getChildren();
	std::vector<Node*>::const_iterator itr=grandChildren.begin();
     
	for (;itr!=grandChildren.end();++itr){
		if ((*itr)->getType()!=SOURCE)
			return false;
 ...
 void Subgraph::addGrandChildrenToExpandedNodes(Node* child){
   std::vector<Node*> grandChildren=child->getChildren();
	std::vector<Node*>::const_iterator itr=grandChildren.begin();
     
	for (;itr!=grandChildren.end();++itr){
		Node* grandChild=*itr;
		m_expandedNodes.insert(grandChild);
