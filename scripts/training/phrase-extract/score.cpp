...
void outputPhrasePair( vector< PhraseAlignment* > &phrasePair, float totalCount ) 
{
  if (phrasePair.size() == 0) return;
	...
	// compute count
	float count = 0;
	size_t countHieroNonTerm = 0, countSyntaxNonTerm = 0;
	...
	for(int j=0;j<phraseT.size();j++)
	{
		...
		if (j < (phraseT.size() - 1))
		{ // not lhs
			string word = vcbT.getWord( phraseT[j] );
			if (word.substr(0, 1) == "[" && word.substr(word.size() - 1, 1) == "]")
			{
				if (word == "[X][X]")
					countHieroNonTerm++;
				else
	...
	// non-term ff
	phraseTableFile << " " << exp(countHieroNonTerm) << " " << exp(countSyntaxNonTerm);
	
	// source lhs
	string sourceLHS = vcbS.getWord( phraseS[phraseS.size() - 1] );
	if (sourceLHS == "[X]")
		phraseTableFile << " 1";
	else 
		phraseTableFile << " 2.718";
