...
int main(int argc, char* argv[]) 
{
  ...
  if (argc != 5 && argc != 6) {
    cerr << "syntax: phrase-score extract lex phrase-table discount [inverse]\n";
    exit(1);
  }
  char* &fileNameExtract = argv[1];
  char* &fileNameLex = argv[2];
  char* &fileNamePhraseTable = argv[3];
	discount = atof(argv[4]);
  inverseFlag = false;
  if (argc > 5) {
    inverseFlag = true;
...
*/
		double scoreNominator = (double) i->second;
		double scoreDenominator = (double) phrasePair.size();

		double phraseScore = .0;
		if(scoreNominator > discount)
			phraseScore = (scoreNominator - discount) / scoreDenominator;

		phraseScore += discount * distinctEnglish * lexScore / scoreDenominator;

		phraseTableFile	<< phraseScore << " " << lexScore;
