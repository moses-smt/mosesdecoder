...
class Lattice
{
	...
	void CreateArcsUsing1Hole(const Tunnel &hole, const SentenceAlignment &sentence, const Global &global);

public:
	Lattice(size_t sourceSize);
	~Lattice();
	
	void CreateArcs(size_t endPos, const TunnelCollection &holeColl, const SentenceAlignment &sentence, const Global &global);
