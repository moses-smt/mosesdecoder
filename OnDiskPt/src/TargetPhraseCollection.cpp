...
void TargetPhraseCollection::Save(OnDiskWrapper &onDiskWrapper)
{
	...
	size_t memUsed = sizeof(Moses::UINT64);
	char *mem = (char*) malloc(memUsed); 

	// size of coll
	Moses::UINT64 numPhrases = GetSize();
	((Moses::UINT64*)mem)[0] = numPhrases;
	...
	//((Moses::UINT64*)mem)[0] = (Moses::UINT64) memUsed;
...
void TargetPhraseCollection::ReadFromFile(size_t tableLimit, Moses::UINT64 filePos, OnDiskWrapper &onDiskWrapper)
{
	...
	Moses::UINT64 numPhrases;

	Moses::UINT64 currFilePos = filePos;
	fileTPColl.seekg(filePos);	
	fileTPColl.read((char*) &numPhrases, sizeof(Moses::UINT64));
	
	// table limit
	numPhrases = std::min(numPhrases, (Moses::UINT64) tableLimit);
	
	currFilePos += sizeof(Moses::UINT64);
	
	for (size_t ind = 0; ind < numPhrases; ++ind)
	{
		TargetPhrase *tp = new TargetPhrase(numScores);
		
		Moses::UINT64 sizeOtherInfo = tp->ReadOtherInfoFromFile(currFilePos, fileTPColl);
...
Moses::UINT64 TargetPhraseCollection::GetFilePos() const
{ return m_filePos; }
