...
char *TargetPhrase::WriteToMemory(OnDiskWrapper &onDiskWrapper, size_t &memUsed) const
{
	...
	size_t memNeeded = sizeof(Moses::UINT64)						// num of words
										+ targetWordSize * phraseSize;	// actual words. lhs as last words
	memUsed = 0;
	Moses::UINT64 *mem = (Moses::UINT64*) malloc(memNeeded); 
...
void TargetPhrase::Save(OnDiskWrapper &onDiskWrapper)
{
	...
	Moses::UINT64 startPos = file.tellp();
...
char *TargetPhrase::WriteOtherInfoToMemory(OnDiskWrapper &onDiskWrapper, size_t &memUsed) const
{
	...
	size_t memNeeded = sizeof(Moses::UINT64); // file pos (phrase id)
	memNeeded += sizeof(Moses::UINT64) + 2 * sizeof(Moses::UINT64) * numAlign; // align
	...
	// phrase id
	memcpy(mem, &m_filePos, sizeof(Moses::UINT64));
	memUsed += sizeof(Moses::UINT64);
...
size_t TargetPhrase::WriteAlignToMemory(char *mem) const
{
	...
	// num of alignments
	Moses::UINT64 numAlign = m_align.size();
...
Moses::UINT64 TargetPhrase::ReadOtherInfoFromFile(Moses::UINT64 filePos, std::fstream &fileTPColl)
{
	assert(filePos == fileTPColl.tellg());
	
	Moses::UINT64 memUsed = 0;
	fileTPColl.read((char*) &m_filePos, sizeof(Moses::UINT64));
	memUsed += sizeof(Moses::UINT64);
	assert(m_filePos != 0);
...
Moses::UINT64 TargetPhrase::ReadFromFile(std::fstream &fileTP, size_t numFactors)
{
	Moses::UINT64 bytesRead = 0;

	fileTP.seekg(m_filePos);

	Moses::UINT64 numWords;
	fileTP.read((char*) &numWords, sizeof(Moses::UINT64));
	bytesRead += sizeof(Moses::UINT64);
...
Moses::UINT64 TargetPhrase::ReadAlignFromFile(std::fstream &fileTPColl)
{
	Moses::UINT64 bytesRead = 0;
	
	Moses::UINT64 numAlign;
	fileTPColl.read((char*) &numAlign, sizeof(Moses::UINT64));
	bytesRead += sizeof(Moses::UINT64);
	
	for (size_t ind = 0; ind < numAlign; ++ind)
	{
		AlignPair alignPair;
		fileTPColl.read((char*) &alignPair.first, sizeof(Moses::UINT64));
		fileTPColl.read((char*) &alignPair.second, sizeof(Moses::UINT64));
		m_align.push_back(alignPair);
		
		bytesRead += sizeof(Moses::UINT64) * 2;
	}
	
	return bytesRead;
}

Moses::UINT64 TargetPhrase::ReadScoresFromFile(std::fstream &fileTPColl)
{
