...
Cache PhraseNode::s_cache;

size_t PhraseNode::GetNodeSize(size_t numChildren, size_t wordSize, size_t countSize)
{
	size_t ret = sizeof(Moses::UINT64) * 2 // num children, value
							+ (wordSize + sizeof(Moses::UINT64)) * numChildren // word + ptr to next source node
...
PhraseNode::PhraseNode(Moses::UINT64 filePos, OnDiskWrapper &onDiskWrapper)
...
{ // load saved node
	...
	file.read((char*) &m_numChildrenLoad, sizeof(Moses::UINT64));
	...
	// get value
	m_value = ((Moses::UINT64*)m_memLoad)[1];
	
	// get counts
	float *memFloat = (float*) (m_memLoad + sizeof(Moses::UINT64) * 2);
...
void PhraseNode::Save(OnDiskWrapper &onDiskWrapper, size_t pos, size_t tableLimit)
{
	...
	for (iter = m_children.begin(); iter != m_children.end(); ++iter)
	{
		...
		Moses::UINT64 *memArray = (Moses::UINT64*) (mem + memUsed);
		memArray[0] = childNode.GetFilePos();
		memUsed += sizeof(Moses::UINT64);
	...
	Moses::UINT64 endPos = file.tellp();
...
void Cache::Add(Moses::UINT64 filePosParent, const Word &wordSought, Moses::UINT64 filePosChild)
{
	if (m_coll.size() > MAX_PRUNE)
	{
		m_coll.clear();
		cerr << "prune\n";
	}
	CacheKey key(filePosParent, wordSought);
	m_coll[key] = filePosChild;
}
	
bool Cache::Get(Moses::UINT64 filePosParent, const Word &wordSought, Moses::UINT64 &filePosChild)
{
	CacheKey key(filePosParent, wordSought);
	
	CacheNode::const_iterator iterCache;
	iterCache = m_coll.find(key);
	if (iterCache != m_coll.end())
	{
	    //cerr << "use cache\n";
		filePosChild= iterCache->second;
		return true;
	}
	else 
	{
...
const PhraseNode *PhraseNode::GetChild(const Word &wordSought, OnDiskWrapper &onDiskWrapper) const
{
  const PhraseNode *ret = NULL;
	...
	Moses::UINT64 childPos;
	bool cacheExist = s_cache.Get(m_filePos, wordSought, childPos); 
	if (cacheExist)
	{
		if (childPos)
			ret = new PhraseNode(childPos, onDiskWrapper);
		return ret;
	}
	...
	if (ret)
		s_cache.Add(m_filePos, wordSought, ret->GetFilePos());
	else 
		s_cache.Add(m_filePos, wordSought, 0);

		
	return ret;	
}

void PhraseNode::GetChild(Word &wordFound, Moses::UINT64 &childFilePos, size_t ind, OnDiskWrapper &onDiskWrapper) const
{

	size_t wordSize = onDiskWrapper.GetSourceWordSize();
	size_t childSize = wordSize + sizeof(Moses::UINT64);
	size_t numFactors = onDiskWrapper.GetNumSourceFactors();

	char *currMem = m_memLoad 
								+ sizeof(Moses::UINT64) * 2 // size & file pos of target phrase coll
...
size_t PhraseNode::ReadChild(Word &wordFound, Moses::UINT64 &childFilePos, const char *mem, size_t numFactors) const
{
	size_t memRead = wordFound.ReadFromMemory(mem, numFactors);
	
	const char *currMem = mem + memRead;
	Moses::UINT64 *memArray = (Moses::UINT64*) (currMem);
	childFilePos = memArray[0];
	
	memRead += sizeof(Moses::UINT64);
	return memRead;
}
