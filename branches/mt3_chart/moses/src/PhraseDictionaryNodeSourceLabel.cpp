// $Id: PhraseDictionaryNodeSourceLabel.cpp 2029 2009-01-28 22:11:23Z hieuhoang1972 $
...
namespace Moses
{
size_t	PhraseDictionaryNodeSourceLabel::s_id = 0;
	
PhraseDictionaryNodeSourceLabel::~PhraseDictionaryNodeSourceLabel()
{
	delete m_targetPhraseCollection;
}

void PhraseDictionaryNodeSourceLabel::CleanUp()
{
	delete m_targetPhraseCollection;
...
void PhraseDictionaryNodeSourceLabel::Sort(size_t tableLimit)
{
	// recusively sort
...
PhraseDictionaryNodeSourceLabel *PhraseDictionaryNodeSourceLabel::GetOrCreateChild(const Word &word, const Word &sourcelabel)
{
	...
	insResult = innerNodeMap->insert( std::make_pair(word, PhraseDictionaryNodeSourceLabel()) );
	assert(insResult.second);

	iter = insResult.first;
	PhraseDictionaryNodeSourceLabel &ret = iter->second;
	ret.SetSourceWord(iter->first);
	//ret.SetSourceWord(word);
	return &ret;
}
	
const PhraseDictionaryNodeSourceLabel *PhraseDictionaryNodeSourceLabel::GetChild(const Word &word, const Word &sourcelabel) const
{
...
void PhraseDictionaryNodeSourceLabel::SetWeightTransModel(const PhraseDictionary *phraseDictionary
...
std::ostream& operator<<(std::ostream &out, const PhraseDictionaryNodeSourceLabel &node)
{
	out << *node.GetTargetPhraseCollection();
	return out;
}

TO_STRING_BODY(PhraseDictionaryNodeSourceLabel)
