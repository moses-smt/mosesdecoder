...
bool Vocab::Load(OnDiskWrapper &onDiskWrapper)
{
	...
	while(getline(file, line))
	{
		...
		m_vocabColl[key] =  Moses::Scan<Moses::UINT64>(tokens[1]);
	}
...
Moses::UINT64 Vocab::AddVocabId(const std::string &factorString)
{
	// find string id
...
Moses::UINT64 Vocab::GetVocabId(const std::string &factorString, bool &found) const
{
	// find string id
	CollType::const_iterator iter = m_vocabColl.find(factorString);
	if (iter == m_vocabColl.end())
