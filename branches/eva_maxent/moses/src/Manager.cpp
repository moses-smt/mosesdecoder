// $Id: Manager.cpp 1897 2008-10-08 23:51:26Z hieuhoang1972 $
// vim:tabstop=2
...
void OutputWordGraph(std::ostream &outputWordGraphStream, const Hypothesis *hypo, size_t &linkId)
{
	...
	for (iterPhraseTable = phraseTables.begin() ; iterPhraseTable != phraseTables.end() ; ++iterPhraseTable)
	{
			...
			const std::vector<LexicalReordering*> &lexOrderings = staticData.GetLexicalReorderModels();
			...
			// maxent re-ordering
			const std::vector<MaxentReordering*> &maxentOrderings = staticData.GetMaxentReorderModels();
			std::vector<MaxentReordering*>::const_iterator iterMaxentOrdering;
			for (iterMaxentOrdering = maxentOrderings.begin() ; iterMaxentOrdering != maxentOrderings.end() ; ++iterMaxentOrdering)
			{
				MaxentReordering *maxentReordering = *iterMaxentOrdering;
				vector<float> scores = hypo->GetScoreBreakdown().GetScoresForProducer(maxentReordering);
