...
protected:
	...
	const NGramNode* GetLmID( FACTOR_ID factor ) const
	{
		size_t factorId = factor->GetId();
		return ( factorId >= m_lmIdLookup.size()) ? NULL : m_lmIdLookup[factorId];        
  };

	float GetValue(FACTOR_ID factor0, State* finalState) const;
	float GetValue(FACTOR_ID factor0, FACTOR_ID factor1, State* finalState) const;
	float GetValue(FACTOR_ID factor0, FACTOR_ID factor1, FACTOR_ID factor2, State* finalState) const;
