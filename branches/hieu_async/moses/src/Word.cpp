// $Id: Word.cpp 160 2007-10-22 13:29:41Z hieu $
// vim::tabstop=2
...
int Word::Compare(const Word &targetWord, const Word &sourceWord)
{
	for (FactorType factorType = 0 ; factorType < MAX_NUM_FACTORS ; factorType++)
	{
		...
		const Factor *unkFactor = FactorCollection::Instance().AddFactor(Output, factorType, UNKNOWN_FACTOR);
		if (targetFactor == unkFactor || sourceFactor == unkFactor)
			continue;
...
void Word::Merge(const Word &sourceWord)
{
	for (FactorType currFactor = 0 ; currFactor < MAX_NUM_FACTORS ; currFactor++)
	{
		...
		const Factor *unkFactor = FactorCollection::Instance().AddFactor(Output, currFactor, UNKNOWN_FACTOR);

		if ((targetFactor == NULL || targetFactor == unkFactor) && sourcefactor != NULL)
		{
