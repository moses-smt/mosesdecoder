 // $Id: Phrase.cpp 145 2007-10-14 18:21:46Z hieu $
// vim:tabstop=2
...
bool Phrase::IsCompatible(const Phrase &inputPhrase, size_t startPos) const
{
	size_t thisPos = startPos;
	for (size_t inputPos = 0 ; inputPos < inputPhrase.GetSize() ; inputPos++)
	{
		if (thisPos >= GetSize())
			return true;
		if (Word::Compare(GetWord(thisPos), inputPhrase.GetWord(inputPos)))
			return false;
