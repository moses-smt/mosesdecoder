// $Id: PDTAimp.cpp 118 2007-09-25 13:28:21Z hieu $
...
void PDTAimp::Factors2String(Word const& w,std::string& s) const 
{
...
void PDTAimp::AddEquivPhrase(const Phrase &source, const TargetPhrase &targetPhrase) 
{
	...
	for(size_t i=0;i<cands.size();++i) 
		{
			...
			StringTgtCand::first_type const& factorStrings=cands[i].GetTargetPhrase();
			StringTgtCand::second_type const& probVector=cands[i].GetScoreComponent();
...
TargetPhraseCollection* PDTAimp::PruneTargetCandidates(std::vector<TargetPhrase> const & tCands,
							...
							if(tcands.size()) 
								{
									...
									{
										std::vector<float> nscores(tcands[i].GetScoreComponent().size()+m_numInputScores,0.0);
										...
										std::transform(tcands[i].GetScoreComponent().begin(),tcands[i].GetScoreComponent().end(),nscores.begin() + m_numInputScores,TransformScore);
										
										assert(nscores.size()==m_weights.size());
										float score=std::inner_product(nscores.begin(), nscores.end(), m_weights.begin(), 0.0f);

										score-=tcands[i].GetTargetPhrase().size() * m_weightWP;
										std::pair<E2Costs::iterator,bool> p=e2costs.insert(std::make_pair(tcands[i].GetTargetPhrase(),TScores()));
