// $Id: WordsBitmap.cpp 158 2007-10-22 00:47:01Z hieu $
	...
	:m_size	(size)
{
	...
	m_bitmap.resize(decodeStepList.GetSize());
...
bool WordsBitmap::IsComplete(FactorType factorType) const
{
	for (size_t decodeStepId = 0 ; decodeStepId < m_bitmap.size() ; ++decodeStepId)
	{
		...
		const FactorMask &outputFactorMask = decodeStep.GetNonConflictFactorMask();
...
bool WordsBitmap::IsComplete(const DecodeStepTranslation &decodeStep) const
{
