// $Id: Hypothesis.h 176 2007-11-02 19:29:01Z hieu $
// vim:tabstop=2
...
protected:
	...
	std::vector<const Hypothesis*> m_backPtrHypo; /**< backpointer to previous hypothesis (from which this one was created) */
  ...
  bool							m_wordDeleted;
	...
	float CalcFutureScore(const SpanScore &futureScore);
	...
	Hypothesis(InputType const& source, const DecodeStepCollection &decodeStepList, const TargetPhrase &emptyTarget);
...
public:
	...
	static Hypothesis* Create(InputType const& source, const DecodeStepCollection &decodeStepList, const TargetPhrase &emptyTarget);
	...
	void CalcScore(const SpanScore &futureScoreSpan);
	...
	*/
	const Word *GetWord(size_t pos, const DecodeStepTranslation &decodeStep) const;
...
//	float GetFutureScore() const { return m_futureScore; }
	...
	// target sizes for every decode step are the same
	bool IsSynchronized() const;
	// same current target phrase, & prev hypo pointers for every decode step are the same
	bool IsIdentical(const Hypothesis &other) const;
	...
	size_t GetTargetSize(const DecodeStepTranslation &decodeStep) const
	{
		return m_targetSize[decodeStep.GetId()];
	}
	const Hypothesis *GetPrevHypo(const DecodeStepTranslation &decodeStep) const
	{
		return m_backPtrHypo[decodeStep.GetId()];
	}

	void DeleteSourceBitmap();
	bool OverlapPreviousStep(size_t decodeStepId, size_t startPos, size_t endPos) const;
