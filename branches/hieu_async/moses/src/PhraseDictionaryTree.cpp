// $Id: PhraseDictionaryTree.cpp 116 2007-09-23 23:46:22Z hieu $
// vim:tabstop=2
...
class TgtCand {
	IPhrase e;
	Scores sc;
	std::vector<std::string> m_sourceAlignment, m_targetAlignment;
public:
	TgtCand() {}
	TgtCand(const IPhrase& a, const Scores& b
		, const std::vector<std::string> &sourceAlignment, const std::vector<std::string> &targetAlignment) 
		: e(a)
		, sc(b)
	...
	void writeBin(FILE* f) const 
	{
		fWriteVector(f,e);
		fWriteVector(f,sc);
		fWriteVector(f, m_sourceAlignment);
		fWriteVector(f, m_targetAlignment);
	}
	void readBin(FILE* f) 
	{
		fReadVector(f,e);
		fReadVector(f,sc);
		fReadVector(f, m_sourceAlignment);
		fReadVector(f, m_targetAlignment);
	}	
};
		...
		for(TgtCands::const_iterator i=tcands.begin();i!=tcands.end();++i)
			{
				...
				StringTgtCand targetCand(vs,i->GetScores());
				rv.push_back(targetCand);
			}
	}
	...
	while(getline(inFile, line)) 	
		{
			...
			std::vector<std::string> sourceAlignment, targetAlignment;

			while(is>>w && w!="|||") f.push_back(imp->sv.add(w));
			while(is>>w && w!="|||") e.push_back(imp->tv.add(w));
			
			while(is>>w && w!="|||") 
				sourceAlignment.push_back(w);
			while(is>>w && w!="|||") 
				targetAlignment.push_back(w);
