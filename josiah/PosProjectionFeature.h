...
class PosProjectionFeature: public FeatureFunction {
    public:
        PosProjectionFeature(Moses::FactorType sourceFactorType, size_t numValues)
            : 
              m_sample(NULL),
...
class PosProjectionBigramFeature : public PosProjectionFeature {
    public:
        ...
        PosProjectionBigramFeature(Moses::FactorType sourceFactorType,
            ...
            PosProjectionFeature(sourceFactorType,tags.size()*tags.size())
            {
                for (std::vector<std::string>::const_iterator ti = tags.begin();
                    ti != tags.end(); ++ti) {
                    m_tags[*ti] = ti-tags.begin();
                    for (std::vector<std::string>::const_iterator tj = tags.begin();
                         tj != tags.end(); ++tj) {
                           std::ostringstream name;
                           name << *ti << ":" << *tj;
                           m_names.push_back(FName("ppbf",name.str()));
                         }
                }
            }
