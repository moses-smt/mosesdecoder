  ...
  class OnlineLearner {
    public :	
			OnlineLearner(const FVector& initWeights, const std::string& name) : 
              m_currWeights(initWeights), m_cumulWeights(initWeights), m_name(name), m_iteration(0) {} //, m_averaging(true)
      virtual void doUpdate(TranslationDelta* curr, TranslationDelta* target, TranslationDelta* noChangeDelta, Sampler& sampler)  = 0;
      void UpdateCumul() ;
      const FVector& GetCurrWeights() { return m_currWeights; }
  ...
  class PerceptronLearner : public OnlineLearner {
    ...
    private:
        ...
        void updateMean(FValue alpha, FValue y) {
            m_currWeights += alpha*y*m_currSigmaDiag*m_features;
		}
