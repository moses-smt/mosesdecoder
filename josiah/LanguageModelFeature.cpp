...
namespace Josiah {
  ...
  LanguageModelFeature::LanguageModelFeature(const LanguageModel* lmodel):
      SingleValuedFeatureFunction(lmodel->GetScoreProducerDescription()),
  m_lmodel(lmodel),
  m_sample(NULL) {}
  
  
  void LanguageModelFeature::init(const Sample& sample) {
    m_sample = &sample;
  }
  
  /** Compute total score for sentence */
  FValue LanguageModelFeature::computeScore() {
    FValue score = 0;
    size_t order = m_lmodel->GetNGramOrder();
    vector<const Word*> lmcontext;
    const vector<Word>& target = m_sample->GetTargetWords();
    lmcontext.reserve(target.size() + 2*(order-1));
    for (size_t i = 0; i < order-1; ++i) {
      lmcontext.push_back(&(m_lmodel->GetSentenceStartArray()));
    }
    for (size_t i = 0; i < target.size(); ++i) {
      lmcontext.push_back(&(target[i]));
    }
    lmcontext.push_back(&(m_lmodel->GetSentenceEndArray()));
  ...
  FValue LanguageModelFeature::getSingleUpdateScore(const Moses::TranslationOption* option, const TargetGap& gap) {
    size_t order = m_lmodel->GetNGramOrder();
    ...
    for (size_t i = 0; i < order-1; ++i) {
      if (start+(int)i < 0) {
        ...
        lmcontext.push_back(&(m_sample->GetTargetWords()[i+start]));
      }
    }
    ...
    for (size_t i = 0; i < order-1; ++i) {
      ...
      if (targetPos >= m_sample->GetTargetWords().size()) {
        if (targetPos == m_sample->GetTargetWords().size()) {
          lmcontext.push_back(&(m_lmodel->GetSentenceEndArray()));
        }
      } else {
        lmcontext.push_back(&(m_sample->GetTargetWords()[targetPos]));
      }
    }
    ...
    for (size_t ngramstart = 0; ngramstart < lmcontext.size() - (order -1); ++ngramstart) {
      if (ngramstart >= startOption && ngramstart + order - 1 < endOption) {
        ...
        lmscore += m_lmodel->GetValue(ngram);
      }  
    }
  ...
  FValue LanguageModelFeature::getContiguousPairedUpdateScore(const TranslationOption* leftOption,
   ...
   FValue LanguageModelFeature::getDiscontiguousPairedUpdateScore(const TranslationOption* leftOption,const TranslationOption* rightOption, 
       const TargetGap& leftGap, const TargetGap& rightGap) 
   {
     ...
     for (size_t i = 0; i < order - 1; i++) {
       ...
       if (rightOffset < 0) {
         lmcontext.push_back(&(m_sample->GetTargetWords()[leftSegmentEndPos + i + 1]));    
       }
       ...
       else if (rightOffset - rightTgtPhrase.GetSize() + rightGap.segment.GetEndPos() + 1 < m_sample->GetTargetWords().size() ) {
         lmcontext.push_back(&(m_sample->GetTargetWords()[(rightOffset - rightTgtPhrase.GetSize() + rightGap.segment.GetEndPos()  + 1)]));  
       }
       else {
         lmcontext.push_back(&(m_lmodel->GetSentenceEndArray()));
         break;
     ...
     for ( ; i > 0 ; --i) {
       if (i > leftOffset + leftGap.segment.GetStartPos()) {                      
         ...
         lmcontext.push_back(&(m_sample->GetTargetWords()[leftOffset - i + leftGap.segment.GetStartPos() ]));
       }                      
       else if ( i > gapSize) {
         if (i - gapSize == 1){
           leftStartPos = lmcontext.size();
         ...
         lmcontext.push_back(&(m_sample->GetTargetWords()[leftGap.segment.GetEndPos() + gapSize - i + 1 ]));
       }
     }  
     ...
     for (size_t i = 0; i < order-1; ++i) {
       if ( i + rightGap.segment.GetEndPos() + 1 < m_sample->GetTargetWords().size() ) {
         lmcontext.push_back(&(m_sample->GetTargetWords()[i + rightGap.segment.GetEndPos() + 1]));         
       }
       else { 
         lmcontext.push_back(&(m_lmodel->GetSentenceEndArray()));
         break;
   ...
   FValue LanguageModelFeature::getFlipUpdateScore(const TranslationOption* leftOption,const TranslationOption* rightOption, 
                                     const TargetGap& leftGap, const TargetGap& rightGap)
   {
