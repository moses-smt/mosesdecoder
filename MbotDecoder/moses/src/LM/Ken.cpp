// $Id: Ken.cpp,v 1.1.1.1 2013/01/06 16:54:18 braunefe Exp $
...
//language model for MBOT
class LanguageModelMBOTStateKenLM : public FFState {
  public:
    LanguageModelMBOTStateKenLM() {}

    //BEWARE : keep ChartState for MBOT lm
    const lm::ngram::ChartState &GetChartState() const { return m_state; }
    lm::ngram::ChartState &GetChartState() { return m_state; }

    int Compare(const FFState& o) const
    {
      const LanguageModelMBOTStateKenLM &other = static_cast<const LanguageModelMBOTStateKenLM&>(o);
      int ret = m_state.Compare(other.m_state);
      return ret;
    }
