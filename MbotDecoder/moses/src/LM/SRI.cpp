// $Id: SRI.cpp,v 1.2 2013/01/14 19:56:00 braunefe Exp $
...
LMResult LanguageModelSRI::GetValue(const vector<const Word*> &contextFactor, State* finalState) const
{
	...
	//std::cerr << "SRLIM : SIZE OF CONTEXT FACTOR : " << contextFactor.size() << std::endl;

   /*std::cerr << "SRLIM : N_GRAM CONTEXT : " ;
  ...
  //std::cout << "SRILM SETUP CONTEXT : " << std::endl;
  for (size_t i = 0 ; i < count - 1 ; i++) {
    //std::cout << "( " << i+1 << " )"<< *contextFactor[count-2-i] << std::endl;
  ...
  //std::cout << "SRILM GET SCORE for ID : " << *contextFactor[count-1] << std::endl;
  ...
  /*std::cout << "SRLIM 2 : "<< std::endl;
   for(itr_context = contextFactor.begin(); itr_context!=contextFactor.end();itr_context++)
                            {
                                std::cout << "CONTEXT FACTOR : " << **itr_context << std::endl;
                            }*/
  //std::cerr << "SLIM RETURNED VALUE : " << ret.score  << std::endl;
  return ret;
}
