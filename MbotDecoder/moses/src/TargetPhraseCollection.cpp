// $Id: TargetPhraseCollection.cpp,v 1.1.1.1 2013/01/06 16:54:17 braunefe Exp $
...
{
  //std::cout << "PRUNING : TABLE LIMIT : " << tableLimit << std::endl;
  ...
  if (adhereTableLimit && m_collection.size() > tableLimit) {
    for (size_t ind = tableLimit; ind < m_collection.size(); ++ind) {
      ...
      //std::cout << "DELETED TARGET PHRASE IN PRUNE : " << *mbotTargetPhrase << std::endl;
      delete mbotTargetPhrase;
    }
...
{
  //std::cout << "SORTING : TABLE LIMIT : " << tableLimit << std::endl;
  ...
  if (adhereTableLimit && m_collection.size() > tableLimit) {
    for (size_t i = tableLimit; i < m_collection.size(); ++i) {
      ...
      //cast to MBOT
      TargetPhraseMBOT *mbotTargetPhrase = static_cast<TargetPhraseMBOT*>(targetPhrase);
      //std::cout << "DELETED TARGET PHRASE IN SORT : " << *mbotTargetPhrase << std::endl;
      delete mbotTargetPhrase;
    }
    m_collection.erase(m_collection.begin()+tableLimit, m_collection.end());
  }
