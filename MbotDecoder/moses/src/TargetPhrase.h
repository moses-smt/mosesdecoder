// $Id: TargetPhrase.h,v 1.1.1.1 2013/01/06 16:54:17 braunefe Exp $
...
protected:
  ...
  // useful to have source lhs too
  Word m_lhsSource;
  // to know if the source lhs is matched
  bool m_matchesSource;

public:

  //checks if target phrase matches a source label
   //bool m_matchesSource;

    TargetPhrase(Phrase sourcePhrase);
    TargetPhrase(std::string out_string,Phrase sourcePhrase);
    TargetPhrase(const Phrase &,Phrase sourcePhrase);
    TargetPhrase(TargetPhrase * toCopy);
    ~TargetPhrase();

    //does a target phrase match a syntax label in source lhs ?
  ...
  //! Method from TargetPhrase to be overloaded in TargetPhraseMBOT
  ...
  virtual void SetAlignmentInfo(const AlignmentInfo *alignmentInfo) {
    m_alignmentInfo = alignmentInfo;
  }

  virtual const AlignmentInfo &GetAlignmentInfo() const {
