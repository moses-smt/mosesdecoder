// $Id: StaticData.cpp,v 1.1.1.1 2013/01/06 16:54:17 braunefe Exp $
// vim:tabstop=2
...
bool StaticData::LoadData(Parameter *parameter)
{
  ...
  // 1 : Yes
  // 2 : No
    if(m_parameter->isParamSpecified("source-label-matching-at-rule-application")) {
      const vector<string> &args = m_parameter->GetParam("source-label-matching-at-rule-application");
      if (args.size() == 1) {
      	m_matchingSourceAtRuleApplication = Scan<int>( m_parameter->GetParam("source-label-matching-at-rule-application")[0] );
      } else {
        UserMessage::Add(string("the translation-details option requires exactly one filename argument"));
        return false;
      }
  ...
  for (size_t i = 0; i < tsConfig.size(); ++i) {
    ...
    if(m_gapPenaltyProducer != NULL)
    m_translationSystems.find(config[0])->second.AddFeatureFunction(m_gapPenaltyProducer);
