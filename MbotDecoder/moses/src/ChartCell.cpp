// $Id: ChartCell.cpp,v 1.1.1.1 2013/01/06 16:54:17 braunefe Exp $
// vim:tabstop=2
/***********************************************************************
  ...
  ,m_manager(manager)
{
  ...
  //std::cout << "new ChartCell() " << m_coverage << std::endl;
}

ChartCell::~ChartCell()
{
  //std::cout << "~ChartCell() " << m_coverage << std::endl;
                                ...
                                , const ChartCellCollection &allChartCells)
{
  ...
  std::cout << "PROCESSING SENTENCE FROM CELL" << std::endl;
  const StaticData &staticData = StaticData::Instance();
  ...
  for (iterList = transOptList.begin(); iterList != transOptList.end(); ++iterList)
  {
	...
	//Check that translation option matches source syntax label
...
void ChartCell::ProcessSentenceWithSourceLabels(const ChartTranslationOptionList &transOptList
                        ,const ChartCellCollection &allChartCells, const InputType &source, size_t startPos, size_t endPos)
{
	std::cerr << "Process Sentence with Source Labels forces match with syntactic constituent in source sentence" << std::endl;
	std::cerr << "Only implemented for MBOT" << std::endl;
}
