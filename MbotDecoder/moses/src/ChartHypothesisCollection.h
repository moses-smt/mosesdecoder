// $Id: ChartHypothesisCollection.h,v 1.1.1.1 2013/01/06 16:54:17 braunefe Exp $
// vim:tabstop=2
/***********************************************************************
...
{
public:
  virtual bool operator()(const ChartHypothesis* hypoA, const ChartHypothesis* hypoB) const {
...
protected:
   ...
   */
  virtual std::pair<HCType::iterator, bool> Add(ChartHypothesis *hypo, ChartManager &manager);
...
public:
  ...
  ~ChartHypothesisCollection();
  virtual bool AddHypothesis(ChartHypothesis *hypo, ChartManager &manager);

  //! remove hypothesis pointed to by iterator but don't delete the object
  virtual void Detach(const HCType::iterator &iter);
  /** destroy Hypothesis pointed to by iterator (object pool version) */
  virtual void Remove(const HCType::iterator &iter);

  virtual void PruneToSize(ChartManager &manager);

  virtual size_t GetSize() const {
    return m_hypos.size();
  }

  virtual size_t GetHypo() const {
    return m_hypos.size();
  }
  ...
  virtual const HypoList &GetSortedHypotheses() const {
