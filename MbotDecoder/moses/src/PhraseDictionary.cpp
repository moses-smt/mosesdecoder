// $Id: PhraseDictionary.cpp,v 1.1.1.1 2013/01/06 16:54:18 braunefe Exp $
// vim:tabstop=2
...
PhraseDictionary* PhraseDictionaryFeature::LoadPhraseTable(const TranslationSystem* system)
{
  ...
  if (m_implementation == Memory) {
    ...
    CHECK(ret);
    return pdm;
    //new : registrate MBOT phrase dictionary
    ...
    PhraseDictionaryMBOT* pdm  = new PhraseDictionaryMBOT(m_numScoreComponent,this);
    bool ret = pdm->Load(GetInput()
                         , GetOutput()
                         , m_filePath
                         , m_weight
...
PhraseDictionaryFeature::~PhraseDictionaryFeature()
{
    //std::cout << "DELETING PHRASE DICTIONARY FEATURE" << std::endl;
}
