...
ChartTreillisDetourQueueMBOT::~ChartTreillisDetourQueueMBOT() {
  RemoveAllInColl(m_queue);
}

void ChartTreillisDetourQueueMBOT::Push(const ChartTreillisDetourMBOT *detour) {
  if (m_capacity == 0 || m_queue.size() < m_capacity) {
    m_queue.insert(detour);
  } else if (detour->GetTotalScoreMBOT() > (*m_queue.rbegin())->GetTotalScoreMBOT()) {
...
const ChartTreillisDetourMBOT *ChartTreillisDetourQueueMBOT::Pop() {
  QueueType::iterator p = m_queue.begin();
  const ChartTreillisDetourMBOT *top = *p;
  m_queue.erase(p);
  return top;
}
