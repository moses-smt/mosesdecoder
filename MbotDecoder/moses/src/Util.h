// $Id: Util.h,v 1.1.1.1 2013/01/06 16:54:18 braunefe Exp $
...
inline void Tokenize(std::vector<std::string> &output
                     ...
                     , const std::string& delimiters = " \t")
{
  //std::cout << "UTIL : STRING TO TOKENIZE : " << str << std::endl;
  // Skip delimiters at beginning.
  std::string::size_type lastPos = str.find_first_not_of(delimiters, 0);
  // Find first "non-delimiter".
  std::string::size_type pos     = str.find_first_of(delimiters, lastPos);
  ...
  while (std::string::npos != pos || std::string::npos != lastPos) {
    ...
    // Found a token, add it to the vector.
    //std::cout << "FOUND 1: " << str.substr(lastPos, pos - lastPos) << std::endl;
    output.push_back(str.substr(lastPos, pos - lastPos));
    // Skip delimiters.  Note the "not_of"
    //std::cout << "PUSHED BACK!" << std::endl;
    //std::cout << "FOUND 2: " << str.find_first_not_of(delimiters, pos) << std::endl;
    lastPos = str.find_first_not_of(delimiters, pos);
    // Find next "non-delimiter"
    //std::cout << "FOUND 3: " << str.find_first_of(delimiters, lastPos) << std::endl;
    pos = str.find_first_of(delimiters, lastPos);
  }
}
...
{
  //std::cerr << "UNTRANSFORMING LM SCORE" << logNScore << std::endl;
  // opposite of above
...
void RemoveAllInColl(COLL &coll)
{
  for (typename COLL::const_iterator iter = coll.begin() ; iter != coll.end() ; ++iter) {
    //std::cout << "Trying to free : " << iter << std::endl;
    delete (*iter);
