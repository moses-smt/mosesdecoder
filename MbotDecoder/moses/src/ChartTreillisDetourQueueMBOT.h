...
class ChartTreillisDetourQueueMBOT {
 public:
  ...
  ChartTreillisDetourQueueMBOT(size_t c) : m_capacity(c) {}
  ...
  void Push(const ChartTreillisDetourMBOT *detour);

  // Remove the best-scoring detour from the queue and return it.  The
  // caller is responsible for deleting the object.
  const ChartTreillisDetourMBOT *Pop();

 private:
  struct DetourOrderer {
    bool operator()(const ChartTreillisDetourMBOT* a,
                    const ChartTreillisDetourMBOT* b) const {
      return (a->GetTotalScoreMBOT() > b->GetTotalScoreMBOT());
    }
  };

  typedef std::multiset<const ChartTreillisDetourMBOT *, DetourOrderer> QueueType;
