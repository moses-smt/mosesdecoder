...
int main(int argc, char* argv[])
{
  ...
  if (argc < 4) {
    cerr << "syntax: consolidate phrase-table.direct phrase-table.indirect phrase-table.consolidated [--Hierarchical] [--Mbot] [--OnlyDirect] \n";
    exit(1);
  }
  ...
  for(int i=4; i<argc; i++) {
    ...
    } else if (strcmp(argv[i],"--Mbot") == 0) {
      mbotFlag = true;
      cerr << "processing mbot rules\n";
...
{
  InputFileStream fileCountOfCounts(fileNameCountOfCounts);
...
void processFiles( char* fileNameDirect, char* fileNameIndirect, char* fileNameConsolidated, char* fileNameCountOfCounts )
{
  ...
  // open input files
  InputFileStream fileDirect(fileNameDirect);
  InputFileStream fileIndirect(fileNameIndirect);
  ...
  OutputFileStream fileConsolidated;
  ...
  while(true) {
    ...
    // do not output additional bars for mbot (decoder will die)
    if ( !mbotFlag ) {
    	fileConsolidated << " |||";
    }
