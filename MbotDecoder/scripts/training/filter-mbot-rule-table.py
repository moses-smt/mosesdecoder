...
def printUsage():
    sys.stderr.write("Usage: filter-mbot-rule-table.py [--min-non-initial-rule-count=N] INPUT")

def main():
    parser = optparse.OptionParser()
    parser.add_option("-c", "--min-non-initial-rule-count",
                      action="store", dest="minCount", type="float", default="0.0",
...
def parseRule(line):
    cols = line.split("|||")
    # NIN: [MBOT] ||| the [JJ] cat [NP] ||| ... => cols[1]!
    rhsSourceSymbols = cols[1].split()[:-1]
    ruleCount = None
    if len(cols) > 5:
        counts = cols[5].split()
        if len(counts) == 3:
