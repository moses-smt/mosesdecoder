...
if (!defined $dir || !defined $config || !defined $input) {
  print STDERR "usage: filter-model-given-input.pl targetdir moses.ini input.text [-Binarizer binarizer] [-Hierarchical|-Mbot]\n";
  exit 1;
}
...
while(<INI>) {
  ...
  if (/ttable-file\]/) {
    while(1) {
      ...
      if ( ($phrase_table_impl ne "0" && $phrase_table_impl ne "6" && $phrase_table_impl ne "11") || $file =~ /glue-grammar/ || $file =~ /glue-mbot-grammar/ ) {
...
sub mk_open_string {
  ...
  } elsif ($opt_hierarchical || $opt_mbot) {
    $openstring = "cat $file |";
  } else {
    $openstring = "< $file";
  }
...
for(my $i=0;$i<=$#TABLE;$i++) {
  ...
  #MBOT
    ...
    print "FILTER-MBOT-RULE-TABLE";
    my $tmp_input = $TMP_INPUT_FILENAME{$factors};
    my $options = "";
    $options .= "--min-non-initial-rule-count=$opt_min_non_initial_rule_count" if defined($opt_min_non_initial_rule_count);
    # NIN: calling filter-mbot-rule-table.py
    open(PIPE,"$openstring /mount/arbeitsdaten21/projekte/emmy-noether/RuleExtraction/scripts/filter-mbot-rule-table.py $options $tmp_input |");
    while (my $line = <PIPE>) {
      print FILE_OUT $line
    }
    close(FILEHANDLE);
