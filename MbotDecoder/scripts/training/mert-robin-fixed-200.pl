...
my @ABBR_FULL_MAP = qw(d=weight-d lm=weight-l tm=weight-t w=weight-w
  g=weight-generation lex=weight-lex I=weight-i gpp=weight-gap);
#my @ABBR_FULL_MAP = qw(d=weight-d lm=weight-l tm=weight-t w=weight-w
...
sub run_decoder {
    ...
    #}
    # begin ROBIN
    # da an run$run.out im decoder aufruf angehängt wird, wird hier eine leere
    # datei run$run.out erstellt, falls sie von einem vorherigen aufruf noch
    # existiert; rest genauso
    # open(OUT_RUN, ">run$run.out");
    # close(OUT_RUN);
    # open(OUT_MERT, ">mert$run");
    # open(OUT_TEMP_NBEST, ">temp-nbest$___N_BEST_LIST_SIZE");
    open(INPUT, "<$___DEV_F");
    my $counter_for_file_partitioning = 0;
    my $partition = 0;
    my $part_size = 200;
    open(PARTITIONED_INPUT, ">temporary-input.txt");
    open(RUN_OUTPUT, ">run$run.out");
    close(RUN_OUTPUT);
    #open(DE_PARTITIONED_NBEST, ">>de-partitioned-nbest");
    while(<INPUT>) {
        #if ($_ =~ /^$/) {print "it's blank\n";}
        $counter_for_file_partitioning += 1;
        print "+1 $counter_for_file_partitioning\n";
        print PARTITIONED_INPUT "$_";
        if ($counter_for_file_partitioning % $part_size == 0) {
            print "$part_size\n";
            $partition = $counter_for_file_partitioning / $part_size;
            if ($___LATTICE_SAMPLES) {
                $lsamp_cmd = " -lattice-samples $lsamp_filename-$partition $___LATTICE_SAMPLES ";
            }
            close(PARTITIONED_INPUT);
            
            # aufruf des decoders mit kleinerem input
            # TODO: -qsub-prefix ??? braucht das für jeden teillauf eine eigene
            # datei? wenn ja -> temp-mert$run jedesmal anlegen, ergebnis an
            # letztendlich anhängen, löschen
            # TODO: -n-best-list: jedesmal in temp-nbest schreiben, an original
            # anhängen
            if (defined $___JOBS && $___JOBS > 0) {
                $decoder_cmd = "$moses_parallel_cmd $pass_old_sge "
		    ."-config $___CONFIG -inputtype $___INPUTTYPE "
		    ."-qsub-prefix mert$run-$partition "
		    ."-queue-parameters \"$queue_flags\" "
		    ."-decoder-parameters \"$___DECODER_FLAGS $decoder_config\" "
		    ."$lsamp_cmd -n-best-list \"$filename-$partition $___N_BEST_LIST_SIZE\" "
		    ."-input-file temporary-input.txt -jobs $___JOBS "
		    ."-decoder $___DECODER >> run$run.out";
            }
            else {
                $decoder_cmd = "$___DECODER $___DECODER_FLAGS  "
		    ."-config $___CONFIG -inputtype $___INPUTTYPE $decoder_config"
		    ." $lsamp_cmd -n-best-list $filename-$partition $___N_BEST_LIST_SIZE "
                ."-input-file temporary-input.txt >> run$run.out";
            }
            print "decoder aufruf 1\n";
            safesystem($decoder_cmd) 
		or die "The decoder died. CONFIG WAS $decoder_config \n";
            open(PARTITIONED_INPUT, ">temporary-input.txt");
        }
    }
    print "vergleich $counter_for_file_partitioning\n";
    if ($counter_for_file_partitioning > 0) {
	print "kleiner100\n";
	...
	# aufruf des decoders falls rest weniger als 100 zeilen und min 1 zeile
	if (defined $___JOBS && $___JOBS > 0) {
	    $decoder_cmd = "$moses_parallel_cmd $pass_old_sge "
		."-config $___CONFIG -inputtype $___INPUTTYPE "
		."-qsub-prefix mert$run-$partition "
    ...
    # iterate over $filename- & $lsamp_filename- partitions and concatenate to one large
    ...
    #foreach $file (@files) {
    my $l = 0;
    open(DATA, "< run$run.out") or die "can't open run$run.out: $!";
    $l++ while <DATA>;
    close(DATA);
    $l = int($l / $part_size) + 1;
    print "asdf $l\n";
    for (my $i=1; $i <= $l; $i++) {
	open(TEMP, "<$filename-$i");
	while(<TEMP>) {
	    my ($head, @tail) = split /\ +/, $_;
	    my $corr_line = $line_number + $head;
	    $_ =~ s/^(\d+)/$corr_line/g;
	    print FILENAME "$_";
	}
	$line_number += $part_size;
	close(TEMP);
    }
    close(FILENAME);
    
    open(LSAMP_FILE, ">>$lsamp_filename");       
    
    #@files = <$lsamp_filename-*>;
    $line_number = 0;
    # foreach $file (@files) {
    #   open(TEMP, "<$file");
    for (my $i=1; $i <= $partition; $i++) {
      open(TEMP, "<$lsamp_filename-$i");
      while(<TEMP>) {
        my ($head, @tail) = split /\ +/, $_;
        my $corr_line = $line_number + $head;
        $_ =~ s/^(\d+)/$corr_line/g;
...
sub get_order_of_scores_from_nbestlist {
  ...
  foreach my $tok (split /\s+/, $scores) {
    if ($tok =~ /.+_.+:/) {
      ...
      $label = $1;
    } elsif ($tok =~ /^-?[-0-9.e+]+$/) {  #NIN: added '+' in char class
      if (!$sparse) {
