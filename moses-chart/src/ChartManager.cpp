...
void Manager::ProcessSentence()
{
	...
	for (size_t width = 1; width <= size; ++width)
	{
		for (size_t startPos = 0; startPos <= size-width; ++startPos)
		{
			...
			//cerr	<< "hypos\n" << cell << endl;
			/*
			m_transOptColl.CreateUnaryTranslationOptionsForRange(startPos, endPos);
			const TranslationOptionList &unaryList = m_transOptColl.GetUnaryTranslationOptionList(range);
			
			cell.ProcessUnaryRules(unaryList
														 ,m_hypoStackColl);
			cell.SortDefaultOnly();
			*/
			
			//cerr << "hypos " << cell << endl;
			//cerr << cell.GetSize() << " ";
...
void Manager::EnumerateTrellis(size_t count, TrellisPathList &ret) const
{
	size_t size = m_source.GetSize();
	if (count == 0 || size == 0)
		return;
	...
	size_t currCount = 0;
	while (currCount < ret.GetSize() && currCount < count)
	{
		const TrellisPath &currPath = ret.Get(currCount);
		
		TrellisPathCollection contenders;
		currPath.CreateDeviantPaths(contenders);	
		
		TrellisPathCollection::const_iterator iter;
		for (iter = contenders.begin(); iter != contenders.end(); ++iter)
		{
			TrellisPath *newPath = *iter;
			ret.Add(newPath);
		}
