...
bool HypothesisCollection::AddHypothesis(Hypothesis *hypo, bool prune)
{
	...
	std::pair<HCType::iterator, bool> addRet = Add(hypo, prune);
	if (addRet.second)
	...
	{ // incoming hypo is better than the one we have
		...
		bool added = Add(hypo, prune).second;
		if (!added)
		{
...
pair<HypothesisCollection::HCType::iterator, bool> HypothesisCollection::Add(Hypothesis *hypo, bool prune)
{
	...
	{ // equiv hypo doesn't exists
		...
		if (prune && m_hypos.size() > 2*m_maxHypoStackSize-1)
		{
			PruneToSize();
		}
		else 
...
void HypothesisCollection::SortHypotheses()
{
	...
	// put into vec & sort
	std::copy(m_hypos.begin(), m_hypos.end(), std::back_inserter(m_hyposOrdered));
	std::sort(m_hyposOrdered.begin(), m_hyposOrdered.end(), ChartHypothesisScoreOrderer());
}

void HypothesisCollection::SortDefaultOnly()
{
	m_hyposOrdered.clear();
	SortHypotheses();
}
