...
{
	const TargetPhrase &targetPhrase = GetCurrTargetPhrase();
...
size_t Hypothesis::CalcPrefix(Phrase &ret, size_t size) const
{
	...
	for (size_t pos = 0; pos < targetPhrase.GetSize(); ++pos)
	{
		...
		{
			ret.AddWord(targetPhrase.GetWord(pos));
			size--;
		}
...
size_t Hypothesis::CalcSuffix(Phrase &ret, size_t size) const
{
	...
	else
	{
		...
		for (int pos = (int) targetPhrase.GetSize() - 1; pos >= 0 ; --pos)
		{
			...
			{
				ret.PrependWord(targetPhrase.GetWord(pos));
				size--;
			}
...
void Hypothesis::CalcLMScore()
{
	...
	const TargetPhrase &targetPhrase = GetCurrTargetPhrase();
	for (size_t targetPhrasePos = 0; targetPhrasePos < targetPhrase.GetSize(); ++targetPhrasePos)
	{
		const Word &targetWord = targetPhrase.GetWord(targetPhrasePos);
