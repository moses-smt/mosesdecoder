...
class htable
{
  ...
  entry2<T>   **table;            //!< hash table  
  int      scan_i;            //!< scan support 
  entry2<T>   *scan_p;            //!< scan support 
  // statistics
 ...
 public:
  ...
  int used(){ return size * sizeof(entry2<T> **) + memory->used(); }
...
htable<T>::htable(int n,int kl){
  ...
  memory=new mempool( sizeof(entry2<T>) , BlockSize );

  table = new entry2<T>* [ size=n ];

  memset(table,0,sizeof(entry2<T> *) * n );
...
T htable<T>::insert(T key){
  ...
  /* not found   */
  if ((q = (entry2<T> *)memory->allocate()) == NULL) /* no room   */
    return NULL;
