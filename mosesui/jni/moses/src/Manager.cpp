// $Id: Manager.cpp 3394 2010-08-10 13:12:00Z bhaddow $
// vim:tabstop=2
...
/*
   ...
   if (hypo->GetId()  > 0) {
     ...
     followingPhrases.insert(followingPhrases.rbegin(), remainingPhrases.rend(), remainingPhrases.end());
...
void OutputWordGraph(std::ostream &outputWordGraphStream, const Hypothesis *hypo, size_t &linkId, const TranslationSystem* system)
{
	...
	for (iterPhraseTable = phraseTables.begin() ; iterPhraseTable != phraseTables.end() ; ++iterPhraseTable)
	{
				...
				vector<float>::const_iterator iterScore = scores.begin();
				++iterScore;
				for ( ; iterScore != scores.end() ; ++iterScore)
				{
...
void Manager::GetWordGraph(long translationId, std::ostream &outputWordGraphStream) const
{
	...
	size_t linkId = 0;
	size_t stackNo = 1;
	std::vector < HypothesisStack* >::const_iterator iterStack = hypoStackColl.begin();
...
void Manager::GetSearchGraph(vector<SearchGraphNode>& searchGraph) const {
  ...
  std::vector < HypothesisStack* >::const_reverse_iterator iterStack;
  for (iterStack = hypoStackColl.rbegin() ; iterStack != hypoStackColl.rend() ; ++iterStack)
  {
    const HypothesisStack &stack = **iterStack;
    HypothesisStack::const_iterator iterHypo;
    for (iterHypo = stack.begin() ; iterHypo != stack.end() ; ++iterHypo)
  ...
  connected[ 0 ] = true;
    std::vector < HypothesisStack* >::const_iterator iterStackFwd;
  for (iterStackFwd = hypoStackColl.begin() ; iterStackFwd != hypoStackColl.end() ; ++iterStackFwd)
  {
    const HypothesisStack &stack = **iterStackFwd;
    HypothesisStack::const_iterator iterHypo;
    for (iterHypo = stack.begin() ; iterHypo != stack.end() ; ++iterHypo)
    {
      const Hypothesis *hypo = *iterHypo;
