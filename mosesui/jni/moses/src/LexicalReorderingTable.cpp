...
void LexicalReorderingTableTree::InitializeForInput(const InputType& input){
  ClearCache();
  if(ConfusionNet const* cn = static_cast<ConfusionNet const*>(&input)){
    Cache(*cn);
  } else if(Sentence const* s = static_cast<Sentence const*>(&input)){
...
void LexicalReorderingTableTree::Cache(const ConfusionNet& input){
  ...
  Sentence const* s = static_cast<Sentence const*>(&input);
  if(!s){
	return;
  }
