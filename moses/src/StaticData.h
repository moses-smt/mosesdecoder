...
public:
	...
	/** Reload some parameter (possibly modified with online commands)
	 * from an already updated Parameter object
	 */
	...
	void PrintInputFactorOrder()
	{
		std::cerr << "StaticData::PrintInputFactorOrder()" << std::endl;
		int i=0;
		for (std::vector<FactorType>::iterator iter = m_inputFactorOrder.begin(); iter!=m_inputFactorOrder.end(); iter++, i++)
			std::cerr << "i:" << i << " -> value:" <<  *iter << std::endl;
		
/*
		for (int i=0;i< m_inputFactorOrder.size(); i++)
			std::cerr << "i:" << i << " -> value:" <<  m_inputFactorOrder[i] << std::endl;
*/
	}

	void ClearInputFactorOrder()
	{
  ...
  inline bool GetDisableDiscarding() const
  {
	...
	const std::vector<float>& GetAllWeights() const{ return m_allWeights; } 
  ...
  float GetLatticeMBRMapWeight() const {return m_lmbrMapWeight;}
	...
	bool GetEnableOnlineCommand() const { return m_enableOnlineCommand; }
