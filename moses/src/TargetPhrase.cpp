...
{
//	std::cerr << "TargetPhrase::SetScore(float score)" << std::endl;
...
//	std::cerr << "Calling TargetPhrase::SetScore(const ScoreProducer* translationScoreProducer,......)" << std::endl;
...
{
//	std::cerr << "TargetPhrase::SetScore(const Scores &scoreVector)" << std::endl;
	//we use an existing score producer to figure out information for score setting (number of scores and weights)
	ScoreProducer* prod = StaticData::Instance().GetPhraseDictionaries()[0];
...
{
//	std::cerr << "TargetPhrase::SetScore(const ScoreProducer* translationScoreProducer,......)" << std::endl;
	assert(weightT.size() == scoreVector.size());
	// calc average score if non-best

//	std::cerr << "weightT: " << weightT << std::endl;
//	std::cerr << "scoreVector: " << scoreVector << std::endl;
  ...
  // Replicated from TranslationOptions.cpp
	...
	const LMList &languageModels2 = StaticData::Instance().GetAllLM();
	
	LMList::const_iterator lmIter;
	//for (lmIter = languageModels.begin(); lmIter != languageModels.end(); ++lmIter)
	for (lmIter = languageModels2.begin(); lmIter != languageModels2.end(); ++lmIter)
	{
...
/*
		std::cerr << " fullScore:" << fullScore 
			<< " weightLM:" << weightLM 
			<< " totalFullScore:" << totalFullScore 
			<< " nGramScore:" << nGramScore 
			<< " totalNgramScore:" << totalNgramScore 
		<< std::endl;
*/
...
//		- (this->GetSize() * weightWP);	 // word penalty
	m_fullScore = m_transScore + totalFutureScore + totalFullScore
		- (this->GetSize() * StaticData::Instance().GetWeightWordPenalty());	 // word penalty

/*
	std::cerr << " m_transScore:" << m_transScore 
		<< " + totalFutureScore:" << totalFutureScore
		<< " + totalFullScore:" << totalFullScore
		<< " - this->GetSize():" << this->GetSize()
		<< " * weightWP:" << StaticData::Instance().GetWeightWordPenalty()
		<< " -> m_fullScore:" << m_fullScore
		<< std::endl;
*/
		//<< " * weightWP:" << weightWP
}
