// $Id: TargetPhrase.cpp 2508 2009-08-16 21:31:06Z hieuhoang1972 $
...
bool TargetPhrase::operator< (const TargetPhrase &compare) const
{
	int cmp = Word::Compare(m_lhsTarget, compare.m_lhsTarget);
	if (cmp != 0)
	{
		return cmp < 0;
	}
	...
	{
		const Phrase &thisPhrase = static_cast<const Phrase&>(*this)
								,&comparePhrase = static_cast<const Phrase&>(compare);
		bool ret = thisPhrase < comparePhrase;
		return ret;
	}
}
