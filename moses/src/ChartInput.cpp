...
int ChartInput::Read(std::istream& in,const std::vector<FactorType>& factorOrder)
{
	...
	for (iterLabel = sourceLabels.begin(); iterLabel != sourceLabels.end(); ++iterLabel)
	{
		...
		sourceNonTerm.SetFactor(0, factor);
		sourceNonTerm.SetIsNonTerminal(true);
		
		AddChartLabelDirect(range.GetStartPos() + 1, range.GetEndPos() + 1, sourceNonTerm);
	}
	
	// default labels
	const Word &defaultSourceNonTerm = staticData.GetInputDefaultNonTerminal();
	const Word &glueSourceNonTerminal = staticData.GetGlueSourceNonTerminal();

	for (size_t startPos = 0; startPos < sourceSize; ++startPos)
	{
		for (size_t endPos = startPos; endPos < sourceSize; ++endPos)
		{
			AddChartLabelOverlap(startPos, endPos, defaultSourceNonTerm);
			AddChartLabelDirect(startPos, endPos, glueSourceNonTerminal);
		}
	}
...
void ChartInput::AddChartLabelDirect(size_t startPos, size_t endPos, const Word &sourceNonTerm)
{	
	LabelList &list = GetLabelList(startPos, endPos);
	list.push_back(sourceNonTerm);
}
	
void ChartInput::AddChartLabelOverlap(size_t startPos, size_t endPos, const Word &sourceNonTerm)
{	
