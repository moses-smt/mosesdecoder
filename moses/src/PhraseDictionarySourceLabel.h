// $Id: PhraseDictionarySourceLabel.h 2451 2009-08-04 13:46:24Z hieuhoang1972 $
// vim:tabstop=2
 ...
 */
class PhraseDictionarySourceLabel : public PhraseDictionary
{
	typedef PhraseDictionary MyBase;
	friend std::ostream& operator<<(std::ostream&, const PhraseDictionarySourceLabel&);
...
protected:
	...
	bool Load(const std::vector<FactorType> &input
					    ...
					    , float weightWP);
	bool LoadBackoff(const std::vector<FactorType> &input
							 , const std::vector<FactorType> &output
							 , std::istream &inBackoffStream
							 , const std::vector<float> &weight
							 , size_t tableLimit
	...
	TargetPhraseCollection &GetOrCreateTargetPhraseCollection(const Phrase &source
																														, const std::vector<Word> &sourceLabels
																														, float entropy, size_t coveredWordInd, const Phrase &coveredWordsPhrase);

	PhraseDictionaryNodeSourceLabel &GetOrCreateNode(const Phrase &source, const std::vector<Word> &sourceLabels, float entropy = 9999);
...
public:
	PhraseDictionarySourceLabel(size_t numScoreComponent)
		: MyBase(numScoreComponent)
		, m_prevSource(Input)
		, m_prevPhraseColl(NULL)
	{
	}
	virtual ~PhraseDictionarySourceLabel();
	...
	bool Load(const std::vector<FactorType> &input
								...
								, const std::string &fileBackoffPath
								, const std::vector<float> &weight
								, size_t tableLimit
								, const LMList &languageModels
						    , float weightWP);
	...
	const ChartRuleCollection *GetChartRuleCollection(InputType const& src
