...
const ChartRuleCollection *PhraseDictionaryMemory::GetChartRuleCollection(InputType const& src
																																					...
																																					, size_t maxTargetSyntaxSpan) const
{
	...
	for (size_t ind = 0; ind < savedNodeColl.size(); ++ind)
	{
		...
		const vector<const Moses::Word*> headWords = cellColl.GetTargetLHSList(WordsRange(startPos, endPos));
