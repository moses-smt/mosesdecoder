// $Id: StaticData.cpp 2525 2009-08-23 21:07:47Z hieuhoang1972 $
// vim:tabstop=2
...
bool StaticData::LoadData(Parameter *parameter)
{
	...
	if (m_parameter->GetParam("verbose").size() == 1)
  {
	...
	SetBooleanParameter( &m_sentenceOnlyNBest, "sentence-only-n-best-list", false );
  ...
  m_timeout_threshold = (m_parameter->GetParam("time-out").size() > 0) ?
	...
	if (!LoadUnaryPhraseTables()) return false;
...
StaticData::~StaticData()
{
	...
	RemoveAllInColl(m_unaryPhraseDictionary);
...
void StaticData::LoadNonTerminals()
{
	...
	// SOURCE
	assert(m_parameter->GetParam("source-non-terminal").size() == 2);	
	vector<string> sourceTokens = m_parameter->GetParam("source-non-terminal");
	string defaultSourceNonTermStr = Trim(sourceTokens[0]);

	// source label overlap
	m_sourceLabelOverlap = (SourceLabelOverlap) Scan<int>(sourceTokens[1]);

	// create non-term words
	m_inputDefaultNonTerminal.SetIsNonTerminal(true);
	const Factor *sourceFactor = factorCollection.AddFactor(Input, 0, defaultSourceNonTermStr);
	m_inputDefaultNonTerminal.SetFactor(0, sourceFactor);
		
	// TARGET
	// used only by unary rules
	assert(m_parameter->GetParam("target-non-terminal").size() == 1);	
	vector<string> targetTokens = m_parameter->GetParam("target-non-terminal");
	string defaultTargetNonTermStr = Trim(targetTokens[0]);
	m_outputDefaultNonTerminal.SetIsNonTerminal(true);
	const Factor *targetFactor = factorCollection.AddFactor(Output, 0, defaultTargetNonTermStr);
	...
	while(getline(inStream, line))
	{
		...
		Word nonTerm;
		nonTerm.SetIsNonTerminal(true);
		const Factor *targetFactor = factorCollection.AddFactor(Output, 0, tokens[0]);
		nonTerm.SetFactor(0, targetFactor);
		
		float score = Scan<float>(tokens[1]); // not used yet
		
		UnknownLHSEntry entry(nonTerm, score);
		m_unknownLHS.push_back(entry);	
	}
	
	// glue rule source
	assert(m_parameter->GetParam("glue-source-non-terminal").size() == 1);
	m_glueSourceNonTerminal.SetIsNonTerminal(true);
	sourceFactor = factorCollection.AddFactor(Output, 0, m_parameter->GetParam("glue-source-non-terminal")[0]);
	m_glueSourceNonTerminal.SetFactor(0, sourceFactor);
...
bool StaticData::LoadPhraseTables()
{
  ...
  assert(m_parameter->GetParam("ttable-file").size() > 0);
    ...
    m_maxNumFactors = std::max(m_maxFactorIdx[0], m_maxFactorIdx[1]) + 1;
		...
		assert(weightAll.size() >= m_tmWeightAllOffset + numScoreComponent);
		...
		for (size_t currScore = 0 ; currScore < numScoreComponent; currScore++)
			weight.push_back(weightAll[m_tmWeightAllOffset + currScore]);
		...
		m_tmWeightAllOffset += numScoreComponent;
		...
		{ // binary phrase table
			VERBOSE(1, "using OnDisk phrase tables for idx "<<currDict<<"\n");
...
bool StaticData::LoadUnaryPhraseTables()
{
	VERBOSE(2,"About to LoadUnaryPhraseTables" << endl);
	...
	const vector<string> &translationVector = m_parameter->GetParam("unary-ttable-file");
    ...
    m_maxNumFactors = std::max(m_maxFactorIdx[0], m_maxFactorIdx[1]) + 1;
		...
		m_unaryPhraseDictionary.push_back(pd);
...
bool StaticData::LoadMapping()
{
	...
	// max spans
	const vector<string> &maxChartSpansVec = m_parameter->GetParam("max-chart-span");
	vector<vector<size_t> > maxChartSpans;
	for (size_t ind = 0; ind < maxChartSpansVec.size(); ++ind)
	{
		vector<size_t>	spanPt	= Tokenize<size_t>(maxChartSpansVec[ind]);
		assert(spanPt.size() == 3);
		maxChartSpans.push_back(spanPt);
	}
	...
	for(size_t i=0; i<mappingVector.size(); i++)
	{
      ...
      if (vectorListIndex > previousVectorList)
      {
		...
		{
			size_t maxDefaultChartSpan = (vectorListIndex < maxChartSpans.size()) ? maxChartSpans[vectorListIndex][0] : DEFAULT_MAX_CHART_SPAN;
			size_t maxSourceSyntaxChartSpan = (vectorListIndex < maxChartSpans.size()) ? maxChartSpans[vectorListIndex][1] : DEFAULT_MAX_CHART_SPAN;
			size_t maxTargetSyntaxChartSpan = (vectorListIndex < maxChartSpans.size()) ? maxChartSpans[vectorListIndex][2] : DEFAULT_MAX_CHART_SPAN;
			DecodeGraph *decodeGraph = new DecodeGraph(maxDefaultChartSpan, maxSourceSyntaxChartSpan, maxTargetSyntaxChartSpan);
...
void StaticData::CleanUpAfterSentenceProcessing() const
{
	...
	for(size_t i=0;i<m_unaryPhraseDictionary.size();++i)
		m_unaryPhraseDictionary[i]->CleanUp();
...
void StaticData::InitializeBeforeSentenceProcessing(InputType const& in) const
{
  ...
  for(size_t i=0;i<m_unaryPhraseDictionary.size();++i) {
		m_unaryPhraseDictionary[i]->InitializeForInput(in);
  }
