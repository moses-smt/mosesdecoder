// $Id: StaticData.cpp 1902 2008-10-14 19:25:18Z hieuhoang1972 $
// vim:tabstop=2
...
StaticData::~StaticData()
{
	...
	RemoveAllInColl(m_lexReorderModels);
	RemoveAllInColl(m_maxentReorderModels);
...
//		std::cerr << "Model " << i << ":";
    ...
    std::string type(ToLower(params[0]));
		...
		if("monotonicity" == type){
			m_lexReorderModels.push_back(new LexicalMonotonicReordering(filePath, mweights, direction, condition, input, output));
		} 
		else if("orientation" == type || "msd" == type)
		{
			m_lexReorderModels.push_back(new LexicalOrientationReordering(filePath, mweights, direction, condition, input, output));
		} 
		else if("directional" == type)
		{
			m_lexReorderModels.push_back(new LexicalDirectionalReordering(filePath, mweights, direction, condition, input, output));
		} 
		else 
		{
			//error unknown type!
...
void StaticData::InitializeBeforeSentenceProcessing(InputType const& in) const
{
  ...
  for(size_t j=0;j<m_lexReorderModels.size();++j){
	m_lexReorderModels[j]->InitializeForInput(in);
  }
  for(size_t j=0;j<m_maxentReorderModels.size();++j){
	m_maxentReorderModels[j]->InitializeForInput(in);
  }
