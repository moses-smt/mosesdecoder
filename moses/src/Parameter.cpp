// $Id: Parameter.cpp 2510 2009-08-17 17:04:49Z hieuhoang1972 $
...
Parameter::Parameter()
{
	...
	AddParam("unary-ttable-file", "location and properties of the UNARY translation tables");
	AddParam("glue-source-non-terminal", "catchall for glue rule when using source syntax. Default to GLUE.SOURCE");
	AddParam("source-non-terminal", "What to label each span, and whether to label it if it already has a label from parser. Default is X 0. 0=add more. 1=replace. 2=discard");
	AddParam("target-non-terminal", "What to label target span. Default is X");
	//AddParam("max-derivations", "Max. number of derivations to fetch back from phrase table for each span. Much needed for target syntax & on-disk phrase table. Default = 200");
	//AddParam("rule-count-threshold", "Mind count needed so that expanding derivations will continue, regardless of max-derivation limit. Default = 20");
	AddParam("sentence-only-n-best-list", "only output the sentence in nbest list, not id or score. Used to get number of devs & unique output. Default = false");
