// $Id: Util.cpp 2416 2009-07-30 11:07:38Z hieuhoang1972 $
...
float GetMemSize()
{ // return mem usage in MB. HACK. works in MacOSX, not tested anywhere else
	pid_t pid = getpid();
	...
	char buff[1000], prevBuff[1000];
	FILE *fp = popen(strme.str().c_str(), "r" );
	assert(fp != NULL);

	while (!feof(fp))
	{
		const char *retVal = fgets( buff, sizeof buff, fp );
		if (retVal != NULL)
		{
			memcpy(prevBuff, buff, 1000);
		}
	}

	pclose( fp );
	
	vector<string> toks = Tokenize(prevBuff);

	float ret = 666;
	if (toks.size() >= 8)
	{
		ret = Scan<float>(toks[7]) / 1024.0f;
	}
	return ret;
}
