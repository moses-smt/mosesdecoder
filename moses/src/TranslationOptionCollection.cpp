...
{
//      std::cerr << "TranslationOptionCollection::ProcessOneUnknownWord........)" << std::endl;
...
//        std::cerr << "Calling TargetPhrase::SetScore(*inputScores)" << std::endl;
		targetPhrase.SetScore(*inputScores);
	} else {
//        std::cerr << "Calling TargetPhrase::SetScore()" << std::endl;
...
//std::cerr << "TranslationOptionCollection::CalcFutureScore()" << std::endl;
...
//std::cerr << "Calling m_futureScore.SetScore(row, col, -numeric_limits<float>::infinity());" << std::endl;
...
//std::cerr << "m_futureScore.SetScore(startPos, endPos, score)" << std::endl;
																													 ...
																													 , bool adhereTableLimit)
{
	if ((StaticData::Instance().GetXmlInputType() != XmlExclusive) || !HasXmlOptionsOverlappingRange(startPos,endPos))
	{
	  Phrase *sourcePhrase = NULL; // can't initialise with substring, in case it's confusion network
		...
		if (useCache)
		{
	VERBOSE(1, "I am using TransOptCache\n");
