// $Id: TranslationOptionCollection.cpp 1902 2008-10-14 19:25:18Z hieuhoang1972 $
// vim:tabstop=2
			...
			for (size_t endPos = startPos ; endPos < startPos + maxSize; endPos++)
			{
				...
				for(iterTransOpt = transOptList.begin() ; iterTransOpt != transOptList.end() ; ++iterTransOpt) 
				{
					...
					if (sourcePhrase)
					{
						...
						if (!score.empty())
							transOpt.CacheLexicalReorderingProb(lexreordering, score);
					}
				}
			}
		}
...
void TranslationOptionCollection::CacheMaxentReordering()
{
	...
	size_t size = m_source.GetSize();
	for (iterMaxentreordering = maxentReorderingModels.begin() ; iterMaxentreordering != maxentReorderingModels.end() ; ++iterMaxentreordering)
	{
		MaxentReordering &maxentreordering = **iterMaxentreordering;
