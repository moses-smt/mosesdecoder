...
{
    //std::cerr << "void PDTAimp::CreateTargetPhrase(TargetPhrase& targetPhrase,........)" << std::endl;
		...
		//std::cerr << "Calling targetPhrase.SetScore(m_obj->GetFeature(), scoreVector, m_weights, m_weightWP, *m_languageModels)" << std::endl;
...
//		targetPhrase.CreateAlignmentInfo("???", "???", 44);
	...
	void SetWeightWP(const float v){ m_weightWP = v; };
/*
	void SetWeight(const vector<float> v){ assert(m_weights.size() == v.size()) ; for (size_t pos = 0 ; pos < v.size() ; pos++) m_weights[pos] = v[pos]; };
*/
