...
std::ostream &operator<<(std::ostream &out, const DottedRuleMBOT &rule)
{
  ...
  if (!rule.IsRootMBOT()) {
    ...
    out << "Size of Cell Label : " << rule.GetChartCellLabelMBOT()->GetLabelMBOT().GetSize() << std::endl;
    out << "Cell Labels : " << rule.GetChartCellLabelMBOT()->GetLabelMBOT().GetSize() << std::endl;
    WordSequence::const_iterator itr_words;
    size_t counter = 1;
        for(itr_words = rule.GetChartCellLabelMBOT()->GetLabelMBOT().begin(); itr_words != rule.GetChartCellLabelMBOT()->GetLabelMBOT().end(); itr_words++)
        {
