...
#include "TranslationModel/PhraseDictionaryCache.h"
			...
			// process xml tag
				...
				if (isClosed || isUnary) {
					...
					// special tag: zone
						...
						/*
						 the cbtm tag provides information to use for the present the future sentences
						...
						CacheBasedLanguageModel* cblm = trans_sys.GetCacheBasedLanguageModel();
						if (cblm)
						{
							...
							if (attributeContent != "")
							{
								VERBOSE(3,"attributeContent:|" << attributeContent << "|" << std::endl);
								vector<string> dlt_cblmcommand_elements = TokenizeMultiCharSeparator(ParseXmlTagAttribute(tagContent,"cblm-command"), "||");
								cblm->Execute(dlt_cblmcommand_elements);
							}
							/*
							 "cblm-file" attribute provides a list of files for the cache-based LM which are loaded sequentially
							 */
							attributeContent = ParseXmlTagAttribute(tagContent,"cblm-file");
							if (attributeContent != "")
							{
								VERBOSE(3,"attributeContent:|" << attributeContent << "|" << std::endl);
								std::vector<std::string> dlt_cblmload_elements = TokenizeMultiCharSeparator(ParseXmlTagAttribute(tagContent,"cblm-file"), "||");
								cblm->Load(dlt_cblmload_elements);
							}
							...
							if (attributeContent != "")
							{
								...
								cblm->Insert(dlt_cblm_elements);
							}
						}
						
						
						// TranslationSystem* trans_sys = StaticData::Instance().GetTranslationSystem(TranslationSystem::DEFAULT);
						// I have to get the pointer to cache based TM
						
						int idx=StaticData::Instance().GetPhraseDictionaryCacheIndex();
						
						if (idx != -1)
						{						
							PhraseDictionaryCache* cbtm = dynamic_cast<PhraseDictionaryCache*>  (StaticData::Instance().GetPhraseDictionaryModels().at(idx)->GetDictionary());		
							
							CHECK(cbtm);
							
							// now that I have PhraseDictionaryCache in cbtm, I proceed
							if (cbtm)
							{
								std::string attributeContent;
								
								/*
								 "cbtm-command" attribute provides commands for the cache-based TM (clean, ...) which are executed sequentially
								 */
								attributeContent = ParseXmlTagAttribute(tagContent,"cbtm-command");
								if (attributeContent != "")
								{
									VERBOSE(3,"attributeContent:|" << attributeContent << "|" << std::endl);
									vector<string> dlt_cbtmcommand_elements = TokenizeMultiCharSeparator(ParseXmlTagAttribute(tagContent,"cbtm-command"), "||||");
									cbtm->Execute(dlt_cbtmcommand_elements);
								}
	                                                	/*
        	                                                "cbtm-file" attribute provides a list of files for the cache-based TM which are loaded sequentially
                	                                        */
                        	                                attributeContent = ParseXmlTagAttribute(tagContent,"cbtm-file");
                                	                        if (attributeContent != "")
                                        	                {
                                                	                VERBOSE(3,"attributeContent:|" << attributeContent << "|" << std::endl);
                                                        	        std::vector<std::string> dlt_cbtmload_elements = TokenizeMultiCharSeparator(ParseXmlTagAttribute(tagContent,"cblm-file"), "||");
                                                                	cblm->Load(dlt_cbtmload_elements);
                                                        	}
	
								
								/*
								 "cbtm" attribute provides information to add to the cache-based TM
								 */
								attributeContent = ParseXmlTagAttribute(tagContent,"cbtm");
								
                                                        	if (attributeContent != "")
                                                        	{
                                                                	VERBOSE(3,"attributeContent:|" << attributeContent << "|" << std::endl);
									std::vector<string> dlt_cbtm_elements = TokenizeMultiCharSeparator(ParseXmlTagAttribute(tagContent,"cbtm"), "||||");
                                                                	cbtm->Insert(dlt_cbtm_elements);
                                                        	}
...
/*
								if (attributeContent != "")
								{
									...
									std::vector<string> dlt_cbtm_elements = TokenizeMultiCharSeparator(ParseXmlTagAttribute(tagContent,"cbtm"), "||||");
									std::vector<string>::iterator itr = dlt_cbtm_elements.begin();
									
									std::vector<std::string> dlt_cbtm_pp = TokenizeMultiCharSeparator((*itr), "|||");
									VERBOSE(1,"trying to insert |" << dlt_cbtm_pp[0] << "|||"<< dlt_cbtm_pp[1] << "|" << std::endl);
									
									cbtm->Insert(dlt_cbtm_pp[0], dlt_cbtm_pp[1]);
									VERBOSE(1,"after Insert |" << dlt_cbtm_pp[0] << "|||"<< dlt_cbtm_pp[1] << "|" << std::endl);
									itr++;
									
									while(itr != dlt_cbtm_elements.end())
									{
										dlt_cbtm_pp.clear();
										dlt_cbtm_pp = TokenizeMultiCharSeparator((*itr), "|||");
										VERBOSE(1,"before Update |" << dlt_cbtm_pp[0] << "|||"<< dlt_cbtm_pp[1] << "|" << std::endl);
										
										cbtm->Insert(dlt_cbtm_pp[0], dlt_cbtm_pp[1], "1");
										VERBOSE(1,"after Update |" << dlt_cbtm_pp[0] << "|||"<< dlt_cbtm_pp[1] << "|" << std::endl);
...
*/
						...
						else{ TRACE_ERR(std::endl << "PhraseDictionaryCache is not set" << std::endl); }
					}
