 ...
 *********************************************************************
*/
//This small library was extended with a simple load and save operations
//to allow storage of bloom filters into binary files
...
public:
   ...
   int load(ifstream& inp){
   	inp.read((char *)&table_size_,sizeof(table_size_));
        if (!inp.good()) return 0;
   	inp.read((char *)&projected_element_count_,sizeof(projected_element_count_));
        if (!inp.good()) return 0;
   	inp.read((char *)&inserted_element_count_,sizeof(inserted_element_count_));
        if (!inp.good()) return 0;
   	inp.read((char *)&random_seed_,sizeof(random_seed_));
        if (!inp.good()) return 0;
   	inp.read((char *)&desired_false_positive_probability_,sizeof(desired_false_positive_probability_));
        if (!inp.good()) return 0;
   	inp.read((char *)&salt_count_,sizeof(salt_count_));
        if (!inp.good()) return 0;
        generate_unique_salt(); //use random seed to generate unique hash functions
   	inp.read((char *)&raw_table_size_,sizeof(raw_table_size_));
        if (!inp.good()) return 0;
   	bit_table_ = new cell_type[static_cast<std::size_t>(raw_table_size_)];
   	inp.read((char *)bit_table_,raw_table_size_); //array of unsigned chars
        if (!inp.good()) return 0;
   	return 1;
   } 

  int save(ofstream& out){
        std::cerr << "saving table_size: " << table_size_ << "\n";
        out.write((char *)&table_size_,sizeof(table_size_));
        std::cerr << "saving projected_element_count_: " << projected_element_count_ << "\n";
        out.write((char *)&projected_element_count_,sizeof(projected_element_count_));
        std::cerr << "saving inserted_element_count_: " << inserted_element_count_<< "\n";
        out.write((char *)&inserted_element_count_,sizeof(inserted_element_count_));
        std::cerr << "saving random_seed_: " << random_seed_ << "\n";
        out.write((char *)&random_seed_,sizeof(random_seed_));
        std::cerr << "saving desired_false_positive_probability_: " << desired_false_positive_probability_ << "\n";
        out.write((char *)&desired_false_positive_probability_,sizeof(desired_false_positive_probability_));
        std::cerr << "saving salt_count_: " << salt_count_ << "\n";
        out.write((char *)&salt_count_,sizeof(salt_count_));
        std::cerr << "saving raw_table_size_: " << raw_table_size_ << "\n";
        out.write((char *)&raw_table_size_,sizeof(raw_table_size_));
        std::cerr << "saving table: \n";
        out.write((char *)bit_table_,raw_table_size_); //array of unsigned chars
        out.flush();
        std::cerr << "done\n";
   ...
   {
      if (bit_table_!=NULL) delete[] bit_table_;
   }
   ...
   inline bool insert(const unsigned char* key_begin, const std::size_t& length)
   {
      ...
      if (new_entry) ++inserted_element_count_;
   ...
   inline bool insert(const T& t)
   {
      ...
      return insert(reinterpret_cast<const unsigned char*>(&t),sizeof(T));
   }

   inline bool insert(const std::string& key)
   {
      return insert(reinterpret_cast<const unsigned char*>(key.c_str()),key.size());
   }

   inline bool insert(const char* data, const std::size_t& length)
   {
      return insert(reinterpret_cast<const unsigned char*>(data),length);
   }
   ...
   inline bloom_filter& operator |= (const bloom_filter& f)
   {
         ...
         )
      {
         ...
         std::cerr<< "Incompatible filters\n";
         std::cerr<< "hash:" << salt_count_ << " " << f.salt_count_ << "\n";
         std::cerr<< "size:" << table_size_ << " " << f.table_size_ << "\n";
         std::cerr<< "key:" << random_seed_ << " " << f.random_seed_ << "\n";
      }
      return *this;
   }
