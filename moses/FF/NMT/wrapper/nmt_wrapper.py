...
class NMTWrapper(object):
    ...
    def build(self):
        ...
        self.enc_dec = RNNEncoderDecoder(self.state, rng, skip_init=True, compute_alignment=True)
        self.enc_dec.build()
    ...
    def get_context_vector(self, source_sentence):
        seq, parsed_in = parse_input(self.state, self.indx_word, source_sentence, idx2word=self.idict_src)
    ...
    def get_prob(self, next_word, c, last_word="", state=None):
        ...
        #  print self.comp_next_probs(c, 0, last_words, *states)
        log_probs = numpy.log(probs)

        best_costs_indices = numpy.array([next_indx])
        #  print best_costs_indices[0], probs[0][best_costs_indices[0]]
        ...
        return probs[0][next_indx], new_states[0]
