...
class KenESMBase {
  public:
    ...
    virtual float SentenceEndScore(const lm::ngram::State&, lm::ngram::State&) const = 0;
...
class KenESM : public KenESMBase {
  public:
    KenESM(const std::string& file)
    ...
    virtual float SentenceEndScore(const lm::ngram::State &in_state,
                                   lm::ngram::State &out_state) const {
        return m_kenlm->Score(in_state, m_kenlm->GetVocabulary().EndSentence(),
                              out_state);
    }
