...
M2ScorerMER::M2ScorerMER(const string& config)
  : StatisticsBasedScorer("M2ScorerMER", config),
    alpha_(Scan<float>(getConfig("alpha", "0.9"))),
...
void M2ScorerMER::setReferenceFiles(const vector<string>& referenceFiles)
{
  ...
  for(size_t i = 1; i < referenceFiles.size(); ++i) {
    std::ifstream in;
    in.open(referenceFiles[i].c_str());
    
    std::vector<std::string> file;
    std::string sentence;
    while(std::getline(in, sentence)) {
      file.push_back(trimStr(sentence));
    }
    references_.push_back(file);
  }
}
...
void M2ScorerMER::prepareStats(size_t sid, const string& text, ScoreStats& entry)
{
  ...
  addMERStats(sentence, sid, stats);
...
float M2ScorerMER::calculateScore(const vector<ScoreStatsType>& comps) const
{
  ...
  float denom = beta_/alpha_ * beta_/alpha_ * p + r;
  if(denom != 0)
    f = (1.0 + beta_/alpha_ * beta_/alpha_) * p * r / denom;
  else
    f = 0.0;
...
float sentenceBackgroundM2MER(const std::vector<ScoreStatsType>& stats, const std::vector<ScoreStatsType>& bg)
{
  ...
  float denom = beta/alpha * beta/alpha * p + r;
  if(denom != 0)
    f = (1.0 + beta/alpha * beta/alpha) * p * r / denom;
  else
    f = 0.0;
  
  float h = stats[3] + bg[3];
  float d = stats[4] + bg[4];
  float i = stats[5] + bg[5];
  float s = stats[6] + bg[6];
  float mer = 1 - (d + i + s)/(h + d + i + s);
  
  float score = alpha * f + (1 - alpha) * mer;
  return score;
}
