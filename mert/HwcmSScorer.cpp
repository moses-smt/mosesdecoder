...
//Rico's class but with no internal tree. Dependency tuples are computed with Stanford Parser instead: dep_id head_id depRel -> 1 2 nsubj
...
//get dependencies from Data.cpp as argument to PrepareStats
/*
...
//	if(this->getConfig("m_modelFileARPA")!="")
//	  	boost::shared_ptr<lm::ngram::Model> (new lm::ngram::Model(this->getConfig("m_modelFileARPA").c_str())).swap(m_WBmodel);
...
void HwcmSScorer::prepareStats(std::size_t sid, const std::string& text, ScoreStats& entry){
  ...
  //if Alignment info is added than this has to be changed
  string depRel = nbestEntry[1];
